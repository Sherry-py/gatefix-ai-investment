...
> **一句话锚点：** 项目方负责「讲故事」，这套标准负责「证据够不够格投」。判断一个 AI 项目能不能投、值多少，靠的不是有没有人背书，是治理证据 + 落地证据在 R/C/O/Ro 四个维度上够不够格——这道分界线，用确定性代码守，不靠 LLM 打分。
...
同一套证据打分规则：治理证据不全 → 投资决策升级投委会；落地证据口径不清 → 估值升级投委会。不是"项目好不好"的主观判断，是"给定一组证据，它够不够格支撑这个不可逆的投资动作"的确定性判定。

## 这是什么

一个**确定性的 AI 项目投资治理闸门**——"项目方说它值 200 亿"和"投委会真的投进去"之间的那道判定层。它回答：当一笔不可逆的资本要投进一个 AI 项目时，谁该被允许往下走、什么时候该停？

答案不该是"项目方讲得够不够动人"，而应该是：
